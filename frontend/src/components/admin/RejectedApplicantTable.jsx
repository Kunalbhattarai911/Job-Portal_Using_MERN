...
const RejectedApplicantTable = () => {
    ...
    return (
        <div>
            <input
                type="text"
                placeholder="Search by name or email or contact"
                value={searchTerm}
                onChange={handleSearchChange}
                className="mb-4 p-2 border border-gray-300 rounded"
            />
            <Table>
                <TableCaption>A list of rejected applicants</TableCaption>
                <TableHeader>
                    <TableRow>
                        ...
                        filteredApplicants.length > 0 ? (
                            filteredApplicants.map((item) => (
                                <TableRow key={item._id}>
                                    ...
                                    <TableCell>{item.status || 'Rejected'}</TableCell>
                                </TableRow>
                            ))
                        ) : (
                            <TableRow>
                                <TableCell colSpan="6">No rejected applicants found.</TableCell>
                            </TableRow>
                        )
                    }
                </TableBody>
